terraform {
  backend "s3" {
    ...
    # key            = "sdp-dev-github-audit-lambda/terraform.tfstate"
    # region         = "eu-west-2"
...
resource "aws_lambda_function" "github_audit_lambda" {
...
resource "aws_cloudwatch_event_rule" "daily_trigger" {
  ...
  description         = "Triggers github audit lambda."
  schedule_expression = var.cron_job_schedule
}

resource "aws_cloudwatch_event_target" "lambda_target" {
  rule      = aws_cloudwatch_event_rule.daily_trigger.name
  target_id = "${var.domain}-${var.service_subdomain}"
  arn       = aws_lambda_function.github_audit_lambda.arn
}
...
resource "aws_lambda_permission" "allow_eventbridge" {
  ...
  function_name = aws_lambda_function.github_audit_lambda.function_name
